...
- **36 tools and 3 guided prompts** over one authoritative catalog in Scripthold `3.1.0`.
...
**Scripthold `3.1.0`** is the current public release, published on 2026-08-18 from the exact commit qualified by the push-event `Test Suite`. It exposes the same 36 tools, 3 guided prompts, 168 registered encodings, and 101 active source-intelligence providers. The GitHub Release contains six raw binaries, six platform archives, six GitHub-produced MCPB bundles, `checksums.txt`, and `mcpb-checksums.txt`; the MCP Registry publishes `io.github.zoster81/scripthold` version `3.1.0`.

The `3.1.0` release contains the completed R23-R28 work. R23-R27 established the 36-tool mutation, filesystem, recovery, and source-intelligence surface delivered in `3.0.0`; R28 adds engine hygiene without changing that public MCP surface. R27 completed on 2026-08-16 with **101 active source-analysis providers across 103 registry rows**; Dockerfile and Make remain auxiliary inactive metadata. `source_symbols` provides bounded declaration/navigation workflows, while `source_query` adds structural search, supported project relations, bounded fingerprint-verified context, and coherent process-local incremental generations. Capability claims are provider-specific and mechanically projected in [docs/LANGUAGE_CAPABILITIES.md](docs/LANGUAGE_CAPABILITIES.md); ambiguous formats fail closed, and no external parser/compiler/LSP runtime or persistent on-disk code index is required.

No milestone after R28 is active by default; Scripthold `3.1.0` publishes the completed R28 engine-hygiene scope on top of the R23-R27 capabilities shipped in `3.0.0`. Current milestone state belongs in [docs/ROADMAP.md](docs/ROADMAP.md), concise completed history in [docs/ROADMAP_HISTORY.md](docs/ROADMAP_HISTORY.md), and the completed subsystem contracts in [docs/MCP_MUTATION_SURFACE.md](docs/MCP_MUTATION_SURFACE.md), [docs/SAFE_FILESYSTEM_OPERATIONS.md](docs/SAFE_FILESYSTEM_OPERATIONS.md), [docs/SOURCE_INTELLIGENCE.md](docs/SOURCE_INTELLIGENCE.md), [docs/BACKUP_RECOVERY.md](docs/BACKUP_RECOVERY.md), [docs/MULTILANGUAGE_CODE_INTELLIGENCE.md](docs/MULTILANGUAGE_CODE_INTELLIGENCE.md), and [docs/ENGINE_HYGIENE.md](docs/ENGINE_HYGIENE.md).
...
`list_encodings` is authoritative for canonical names, aliases, and capability metadata. Scripthold `3.1.0` exposes 168 canonical read/write encodings across Unicode, IBM/DOS/EBCDIC, ISO-8859, Windows, classic Mac/KOI8/other single-byte families, and East Asian/stateful multibyte families.
