...
Version `2.2.0` is the current public Scripthold release, with 30 tools, 3 guided prompts, and 168 registered text encodings over stdio and Streamable HTTP. Its GitHub Release includes six raw binaries, six platform archives, six OS/architecture-specific MCPB bundles, and both checksum manifests; `io.github.zoster81/scripthold` version `2.2.0` is active in the MCP Registry. Version `2.0.0` remains the historical 23-tool rollback baseline and keeps its pre-rebrand assets and Registry identity.

Release `2.2.0` retains the completed R15–R21 durable-task, persistent-backup, offline-diagnostic, and MCP `2026-07-28` work and adds the completed R22 global encoding expansion. The currently deployed Windows amd64 runtime remains `2.1.1`, which is still verified over tunnel-owned stdio plus authenticated legacy/modern HTTP and through active rollback to `2.0.0`; deployment and active rollback verification of `2.2.0` have not been performed.

**R22 completed with `2.2.0` on 2026-08-11.** The release delivers the pinned corpus/provenance gate, authoritative capability registry, complete applicable `golang.org/x/text v0.40.0` surface, full UTF-32 LE/BE text support, 88 deterministic pure-Go single-byte additions, 21 multibyte/stateful or residual exact additions derived and verified against pinned GNU libiconv, conservative detector trust gating, bounded partial-coverage reporting, and the registry-driven all-168-codec public-operation matrix. Representative R22 classes pass malformed-input, cancellation, decoded-limit, deterministic concurrency, bounded-memory, race, static/vulnerability, fuzz, native, container, and six-target release gates. The exact release commit passed the push-event `CI` `Release candidate` gate before the annotated tag was created; GoReleaser, GitHub-only MCPB packaging, and MCP Registry publication then completed successfully. See [docs/GLOBAL_ENCODING_COVERAGE.md](docs/GLOBAL_ENCODING_COVERAGE.md) and [docs/ROADMAP.md](docs/ROADMAP.md) for the completed contract and release record.
