...
- Expanded the authoritative R22 encoding registry from 24 to 57 read/write encodings by exposing the complete applicable `golang.org/x/text v0.40.0` surface: all 46 `charmap.All` single-byte codecs plus EUC-JP, ISO-2022-JP, Shift_JIS, HZ-GB-2312, EUC-KR, and Big5, while keeping UTF-32 LE/BE reserved for its dedicated full-pipeline phase.
- Added strict malformed-input handling across legacy single-byte, multibyte/stateful, GB18030, UTF-8, and UTF-16 paths; undefined bytes and incomplete or illegal encoded sequences now fail instead of silently producing replacement characters. Line-ending mutation now validates the raw encoded stream before commit, uses registry-defined CR/LF code units for EBCDIC and other single-byte formats, and preserves HZ `~\n` continuations as stateful escapes rather than content line endings.
- Added exhaustive phase-3 verification for every pinned `x/text` charmap byte value, registry-filtered IANA/WHATWG aliases, detector-label closure, 57-codec line-ending/edit matrices, stateful chunk boundaries, pinned detection fixtures, and libiconv UTF-8 oracles for HZ-GB-2312 and ISO-2022-JP.
- Promoted UTF-32 LE/BE from BOM-management-only metadata to full read/write encodings, expanding the current R22 source-tree registry from 57 to 59 codecs. UTF-32 now participates in bounded streaming read/write, edit, grep/search decoding, conversion, line-ending detection/conversion, `verify_state`, configured defaults, and BOM policies while generic byte-order-unspecified `utf-32` remains rejected.
- Added strict UTF-32 scalar validation before the pinned `x/text` decoder so truncated units, non-multiple-of-four input, surrogate code points, and values above `U+10FFFF` fail instead of becoming replacement characters. Added conservative BOMless LE/BE detection ahead of UTF-16 with four-byte alignment, dual high-byte NUL evidence, decoded-text quality, scalar validity, and large-file sample/chunked/full coverage.
- Added a shared streaming decoding-error boundary so malformed input rejected during incremental decoder reads is consistently classified as `ENCODING` instead of falling through to `IO_ERROR`; raw line-ending mutation retains its separate byte-preserving validator and no-mutation failure contract.
- Expanded the R22 source-tree registry from 59 to 147 read/write encodings with 88 additional fixed single-byte mappings generated as pure Go from pinned GNU libiconv revision `9d19c66d0a1768cffcf497b2db70bf4018b578d7`. The additions cover further DOS/IBM/EBCDIC code pages plus Atari ST, Georgian, HP Roman8, JIS single-byte variants, KOI8-T, additional classic Mac regional encodings, MuleLao-1, NEXTSTEP, PT154, RISCOS-Latin1, RK1048, TDS565, and VISCII. All 88 remain explicit-only pending detector hardening.
- Added a deterministic maintainer-only libiconv single-byte generator that verifies the pinned source revision, probes each selected converter across all 256 byte values, requires exact one-byte decode and byte-identical reverse encoding, records source-definition and converter-header SHA-256 provenance, and emits checked-in Go tables. Runtime and normal tests remain independent of libiconv, GCC, CGO, subprocesses, and network access.
- Added exhaustive phase-5 verification for all 22,528 generated byte positions, strict undefined-byte rejection, exact representable-rune reverse mapping, source-alias closure, one-byte-chunk streaming, deterministic generator output, five pinned real-world fixture round trips, UTF-8↔IBM737 conversion, CP856 malformed-input no-mutation behavior, registry-wide 147-codec line-ending/edit matrices, and decoder chunk-invariance fuzz coverage for the generated codec path.
- Expanded the R22 source-tree registry from 147 to 168 read/write encodings with 21 phase-6 explicit-only additions from pinned GNU libiconv evidence: Big5-2003; Big5-HKSCS 1999/2001/2004/2008; EUC-CN, EUC-JISX0213, EUC-TW; GB18030:2022; IBM1162/1163; ISO-2022-CN/CN-EXT, ISO-2022-JP-1/-2/-3/-MS, ISO-2022-KR; Johab; Shift_JISX0213; and TCVN. Existing compatibility aliases such as unversioned Big5-HKSCS, CP932/943, CP936/949/950, GB2312, and HZ retain their prior canonical semantics instead of inflating the codec count or silently replacing `x/text` behavior.
- Added a deterministic maintainer-only phase-6 libiconv generator and pure-Go runtime architecture: compact prefix-free direct mappings, bounded ISO-2022 state machines over generated raw character-set tables, bounded TCVN composition lookahead, exact-byte admission checks for IBM1162/1163, and canonical pair handling for HKSCS/X0213. Runtime and normal tests require no libiconv, GCC, CGO, subprocess, or network access.
- Added strict GB18030:2022 support as generated differential overrides on the pinned `x/text v0.40.0` GB18030 codec. The maintainer oracle exhaustively compares all 1,611,540 grammatical two/four-byte sequences and all 1,112,064 non-surrogate Unicode scalar values against pinned GNU libiconv, producing 2,087 decode overrides and 2,087 encode overrides with source-header SHA-256 provenance.
- Added phase-6 oracle, malformed-input, no-mutation, registry-wide line-ending/edit, streaming, and chunk-boundary coverage, including the pinned GNU libiconv TCVN UTF-8 oracle and decoder chunk-invariance fuzzing across EUC-CN, EUC-TW, Johab, ISO-2022, TCVN, and GB18030:2022.
- Hardened R22 automatic detection with registry-aware trust gating: short-input evidence floors, strict-decoder validation, decoded-text quality, control-heavy binary rejection, known-confusion peers, close-candidate comparison, and fail-closed handling when a stronger external detector label has no trusted registry disposition. HZ-GB-2312, ISO-2022-JP, and ISO-2022-KR now require verified stateful syntax, strong raw-detector agreement, strict decoding, and decoded non-ASCII evidence before automatic selection.
- Added phase-7 corpus and fuzz invariants: the 61 pinned detection fixtures produce 29 exact trusted canonical results, one byte-exact/text-equivalent GB18030 payload safely narrowed to its GBK subset, and 31 explicit ambiguities with zero trusted text mismatches; a 50,000-execution fuzz gate proves every trusted BOMless result resolves to an auto-detectable registered codec and strict-decodes successfully. Stateful detection is also verified across the configured chunk boundary and against signature-like ASCII false positives.
- Added phase-8 bounded partial-coverage reporting for `grep_text_files`: valid matches from readable files remain available, while `filesScanned`, `filesSkipped`, `coverageComplete`, ordered `skippedFiles`, and explicit truncation/omitted counts prevent directory results from implying a complete scan when per-file encoding or I/O failures occurred.
- Added additive `encodingErrorCode` refinement for grep and batch file failures, distinguishing ambiguous, malformed, unsupported, BOM-conflicting, unrepresentable, and other encoding failures without changing the stable 2.x top-level `errorCode` vocabulary. Duplicate batch `errors` summaries are now capped by a fixed count and byte budget with explicit omitted counts, while ordered per-file results remain available.
- Added a registry-driven phase-9 public-operation gate for all 168 registered codecs. Every codec now passes explicit write/read/batch/grep, edit preview/apply and byte-identical no-op, patch dry-run/apply/verify, conversion round-trip, line-ending mutation, text verification, and applicable unrepresentable/BOM/JSON checks; trusted automatic detection is also exercised through the complete pinned trusted corpus plus authoritative Unicode BOM witnesses, and all three encoding workflow prompts have explicit contract tests.
- Added phase-10 adversarial/resource verification across representative UTF-32, generated single-byte, direct multibyte, ISO-2022, TCVN, and GB18030:2022 classes: malformed input and decoded limits fail closed, conversion cancellation remains `CANCELLED`, concurrent batch/grep output stays deterministic, 225,000 fixed-count fuzz executions pass, and 1/16/64 MiB bounded-output benchmarks retain approximately 150–155 KiB/op rather than scaling with source size.

### Changed

- Established R22 as the active `2.2.0` milestone for global portable encoding coverage, full UTF-32 text support, conservative detector hardening, pinned libiconv/uchardet corpus-oracle evidence, and complete cross-platform release gates without introducing native charset runtime dependencies.
- Normalized supported encoding aliases through the authoritative registry and pinned IANA/WHATWG indexes while refusing any external alias that would expose an unregistered codec; automatic detection remains deliberately narrower than explicit codec support and follows the registry's `autoDetectable`/`explicitOnly` metadata.
- Changed omitted-encoding `write_whole_file` behavior for existing non-empty files: when conservative detection is ambiguous, the operation now returns `ENCODING_AMBIGUOUS` without mutation instead of silently applying the configured creation default and changing the file's byte representation. New and empty files continue to use the configured default, and explicit encoding remains authoritative.
- Made incremental UTF-8 decoding strict as well as byte-preserving, so malformed UTF-8 now fails with an encoding error instead of being silently replaced with `U+FFFD` during streaming grep, batch read, conversion, and other decoder consumers.
- Preserved cancellation as the stable `CANCELLED` category through incremental decoder/encoder and conversion-preview boundaries instead of allowing context cancellation to be reclassified as an encoding failure.
- Reused fixed transformer scratch buffers in the GB18030:2022 differential wrapper, eliminating one heap allocation per ordinary decoded/encoded sequence while preserving all 2,087 pinned decode and encode overrides.
