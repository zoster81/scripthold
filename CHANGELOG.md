...
- Added the R27 Phase 1 `source_query` contract as the 36th Unreleased tool: one compact read-only `search` / `relations` / `context` surface preserves R25 `source_symbols`, freezes evidence separately from resolved/ambiguous/unresolved/external state, adds fingerprint/index-generation binding plus bounded graph/context limits, and remains explicitly `UNSUPPORTED` for otherwise-valid queries until later native R27 analysis/index phases implement the engine.
