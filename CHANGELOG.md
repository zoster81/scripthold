...
- Reframed the project documentation around its independent dual-transport product scope: stdio and native Streamable HTTP now have equal prominence, while the OpenAI Secure MCP Tunnel is documented as one supported deployment option rather than the project's sole identity.
...
- Added an opt-in stdio compatibility path for intermediaries that probe `server/discover` but then send legacy `initialize` on the same persistent connection. `MCP_STDIO_LEGACY_HANDSHAKE=1` rejects discovery before SDK session state changes, allowing clean legacy fallback without accepting duplicate initialization; modern stdio remains the default and HTTP is unchanged.
- Changed the public OpenAI Secure MCP Tunnel quick start to run Scripthold as an authenticated loopback Streamable HTTP service and bind `tunnel-client` through `MCP_SERVER_URL`. Runtime and discovery bearer headers use an environment reference rather than literal argv values, and launcher success now requires tunnel readiness plus an enabled `main` channel with `probe_status=ok`.
