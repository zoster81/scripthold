...
func addTool[In, Out any](server *mcp.Server, tool *mcp.Tool, typedHandler mcp.ToolHandlerFor[In, Out]) {
	...
	mcp.AddTool(server, tool, handler.WithStructuredErrorOutput(typedHandler))
}
...
func BuildServer(options ServerOptions) *mcp.Server {
	...
	addTool(server, sourceQueryCatalogTool(), handler.Wrap(logger, "source_query", h.SourceQuery))
