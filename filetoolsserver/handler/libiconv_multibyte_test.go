...
func TestPinnedLibiconvFixturesMatchUTF8Oracles(t *testing.T) {
	...
	manifest := loadLibiconvCorpusManifest(t)
...
func TestDetectionFixturesDecodeAndReencodeSemantically(t *testing.T) {
	fixtures := []struct {
		file     string
		encoding string
	}{
...
func TestExplicitConversionRoundTrip(t *testing.T) {
	cases := []struct {
		encoding string
		content  string
	}{
...
func TestMalformedInputFailsClosedWithoutMutation(t *testing.T) {
	cases := []struct {
		name     string
		encoding string
		data     []byte
...
func loadLibiconvCorpusManifest(t *testing.T) internetCorpusManifest {
	t.Helper()
