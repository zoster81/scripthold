...
func TestPublicOperationMatrix_AllSupportedEncodings(t *testing.T) {
	root := t.TempDir()
	...
	for _, item := range registry {
		...
		t.Run(item.Name, func(t *testing.T) {
			...
			assertEncodingMatrixDecodedFile(t, item.Name, path, content)
			...
			if jsonBytes, ok := tryEncodeEncodingMatrix(item.Name, "{\"phase9\":true}\r\n", item.HasBOM); ok {
			...
			if item.HasBOM {
				exerciseEncodingMatrixBOMOperations(t, h, path, item.Name, content)
				exerciseEncodingMatrixBOMDetection(t, h, root, item.Name, content)
			}
		})
	}
}

func assertEncodingMatrixDecodedFile(t *testing.T, encodingName, path, want string) {
	t.Helper()
...
func encodingMatrixUnrepresentableRune(encodingName string) (rune, bool) {
...
func exerciseEncodingMatrixBOMOperations(t *testing.T, h *Handler, path, encodingName, content string) {
	t.Helper()
	ctx := context.Background()
	...
	assertEncodingMatrixDecodedFile(t, encodingName, path, content)
}

func exerciseEncodingMatrixBOMDetection(t *testing.T, h *Handler, root, encodingName, content string) {
	t.Helper()
	path := filepath.Join(root, encodingName+".phase9.bom-detect")
	encoded, ok := tryEncodeEncodingMatrix(encodingName, content, true)
	if !ok {
...
func TestDetectEncoding_TrustedCorpusThroughPublicHandler(t *testing.T) {
...
func tryEncodeEncodingMatrix(encodingName, content string, withBOM bool) ([]byte, bool) {
	var data []byte
	if fileEncoding.IsUTF8(encodingName) {
		data = []byte(content)
	} else {
