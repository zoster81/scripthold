...
func BenchmarkR27Phase17SourceQueryColdWarmAndLookup(b *testing.B) {
	root := b.TempDir()
	const fileCount = 80
...
func BenchmarkR27Phase17SourceQueryMixedLanguageMonorepo(b *testing.B) {
	root := b.TempDir()
...
func phase17BenchmarkFingerprint(b *testing.B, path string) string {
	b.Helper()
...
func BenchmarkR27Phase17SourceQueryLargeGenerated(b *testing.B) {
	root := b.TempDir()
...
func BenchmarkR27Phase17SourceQueryRelationsAndContext(b *testing.B) {
	b.Run("dependency-trace-8-files", func(b *testing.B) {
		...
		input := SourceQueryInput{
			...
			Subject:  &SourceSelectorInput{Kind: "path", Path: startPath, SourceFingerprint: phase17BenchmarkFingerprint(b, startPath)},
			Target:   &SourceSelectorInput{Kind: "path", Path: endPath, SourceFingerprint: phase17BenchmarkFingerprint(b, endPath)},
