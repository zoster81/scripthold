...
func TestPinnedSingleByteFixturesRoundTripExplicitly(t *testing.T) {
	fixtures := []struct {
		file     string
		encoding string
	}{
...
func TestSingleByteConversionRoundTrip(t *testing.T) {
	root := t.TempDir()
...
func TestUndefinedSingleByteFailsClosedWithoutMutation(t *testing.T) {
	root := t.TempDir()
	h := NewHandler([]string{root})
