...
type phase10EncodingClass struct {
	name     string
	encoding string
	text     string
}

var phase10EncodingClasses = []phase10EncodingClass{
...
func TestR22Phase10StreamingCancellationIsTypedAndNonMutating(t *testing.T) {
...
func TestR22Phase10DecodedLimitsApplyAcrossEncodingClasses(t *testing.T) {
	for _, testCase := range phase10EncodingClasses {
		testCase := testCase
		t.Run(testCase.name, func(t *testing.T) {
			root := t.TempDir()
...
func TestR22Phase10MalformedClassRepresentativesFailClosed(t *testing.T) {
	tests := []struct {
		name     string
		encoding string
		data     []byte
