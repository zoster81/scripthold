...
func encodeXTextFixture(t *testing.T, charset, text string) []byte {
	t.Helper()
...
func TestAllRegisteredEncodingsLineEndingRoundTrip(t *testing.T) {
	for _, item := range fileEncoding.ListEncodings() {
		...
		t.Run(item.Name, func(t *testing.T) {
			original := encodeXTextFixture(t, item.Name, "alpha\nbeta\n")
			wantCRLF := encodeXTextFixture(t, item.Name, "alpha\r\nbeta\r\n")
...
func TestStatefulLineEndingRoundTrips(t *testing.T) {
	tests := []struct {
		charset string
		lfText  string
	}{
	...
	for _, testCase := range tests {
		t.Run(testCase.charset, func(t *testing.T) {
			original := encodeXTextFixture(t, testCase.charset, testCase.lfText)
			wantCRLF := encodeXTextFixture(t, testCase.charset, ConvertLineEndings(testCase.lfText, LineEndingCRLF))
...
func TestHZContinuationIsNotAContentLineEnding(t *testing.T) {
	original := []byte("alpha~\nbeta\n")
...
func TestChangeLineEndingsRejectsMalformedSourceWithoutMutation(t *testing.T) {
	tests := []struct {
		charset string
		data    []byte
	}{
...
func TestPinnedCorpusDetection(t *testing.T) {
...
func TestPinnedStatefulCodecOracles(t *testing.T) {
	tests := []struct {
		charset string
		fixture string
		oracle  string
