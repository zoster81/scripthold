...
func TestContextMaterializationRejectsPostPlanMutation(t *testing.T) {
...
func TestSourceQueryContextPreservesDecodedUTF16CoordinatesAndBudget(t *testing.T) {
	root := canonicalHandlerTestDir(t)
	...
	if output.Context.UsedBytes != contextSafetyBytes(output.Context.Items) {
		t.Fatalf("UTF-16 context budget = %+v", output.Context)
	}
}

func contextSafetyBytes(items []sourceintelligence.ContextItem) int {
	total := 0
