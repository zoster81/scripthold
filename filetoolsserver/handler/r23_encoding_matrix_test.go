...
func TestR23MutationSurface_AllSupportedEncodingsPreserveContentAndExactPreparedBytes(t *testing.T) {
	root := t.TempDir()
	...
	for _, item := range registry {
		...
		t.Run(item.Name, func(t *testing.T) {
			...
			assertR23DecodedFile(t, h, ctx, item.Name, path, content)

			initialBytes := mustReadR23Bytes(t, path)
			...
			assertR23BytesEqual(t, path, initialBytes, "edit no-op preview")
			...
			assertR23BytesEqual(t, path, initialBytes, "edit no-op apply")

			editBefore := mustReadR23Bytes(t, path)
			...
			assertR23BytesEqual(t, path, editBefore, "edit preview")
			...
			assertR23Fingerprint(t, path, editPreview.ResultFingerprint, "edit apply")
			...
			patchBefore := mustReadR23Bytes(t, path)
			...
			assertR23BytesEqual(t, path, patchBefore, "patch preview")
			...
			assertR23Fingerprint(t, path, patchPreview.Results[0].ResultFingerprint, "patch apply")
			...
			convertBefore := mustReadR23Bytes(t, path)
			...
			assertR23BytesEqual(t, path, convertBefore, "convert preview")
			...
			assertR23Fingerprint(t, path, convertPreview.ResultFingerprint, "convert apply")
			if actual := mustReadR23Bytes(t, path); !bytes.Equal(actual, []byte(content)) {
				t.Fatalf("UTF-8 conversion bytes differ: got %x want %x", actual, []byte(content))
			}

			backBefore := mustReadR23Bytes(t, path)
			...
			assertR23BytesEqual(t, path, backBefore, "convert-back preview")
			...
			assertR23Fingerprint(t, path, backPreview.ResultFingerprint, "convert-back apply")
			assertR23DecodedFile(t, h, ctx, item.Name, path, content)

			if item.HasBOM {
				exerciseR23BOMCapability(t, h, ctx, item.Name, path, content)
			}
			...
			if unrepresentable, ok := phase9UnrepresentableRune(item.Name); ok {
				...
				assertR23BytesEqual(t, unsupportedPath, original, "unrepresentable conversion preview")
			}
		})
	}
}

func exerciseR23BOMCapability(t *testing.T, h *Handler, ctx context.Context, encodingName, path, content string) {
	t.Helper()
	...
	if detected.HasBOM {
		stripR23BOM(t, h, ctx, encodingName, path, content)
		addR23BOM(t, h, ctx, encodingName, path, content)
		return
	}
	addR23BOM(t, h, ctx, encodingName, path, content)
	stripR23BOM(t, h, ctx, encodingName, path, content)
...
func addR23BOM(t *testing.T, h *Handler, ctx context.Context, encodingName, path, content string) {
	t.Helper()
	before := mustReadR23Bytes(t, path)
	...
	assertR23BytesEqual(t, path, before, "BOM add preview")
	...
	assertR23Fingerprint(t, path, preview.ResultFingerprint, "BOM add apply")
	assertR23DecodedFile(t, h, ctx, encodingName, path, content)
}

func stripR23BOM(t *testing.T, h *Handler, ctx context.Context, encodingName, path, content string) {
	t.Helper()
	before := mustReadR23Bytes(t, path)
	...
	assertR23BytesEqual(t, path, before, "BOM strip preview")
	applyResult, applied, err := h.HandleManageBOMApply(ctx, nil, PreviewApplyInput{PreviewID: preview.PreviewID})
	if err != nil || applyResult.IsError || !applied.Applied || applied.HasBOM {
		t.Fatalf("BOM strip apply result=%+v output=%+v err=%v", applyResult, applied, err)
	}
	assertR23Fingerprint(t, path, preview.ResultFingerprint, "BOM strip apply")
	assertR23DecodedFile(t, h, ctx, encodingName, path, content)
}

func assertR23DecodedFile(t *testing.T, h *Handler, ctx context.Context, encodingName, path, want string) {
	t.Helper()
...
func assertR23Fingerprint(t *testing.T, path, want, operation string) {
	t.Helper()
...
func mustReadR23Bytes(t *testing.T, path string) []byte {
	t.Helper()
...
func assertR23BytesEqual(t *testing.T, path string, want []byte, operation string) {
	t.Helper()
	if got := mustReadR23Bytes(t, path); !bytes.Equal(got, want) {
