...
func TestR27Phase17ConnectorWorkflowAcrossHeterogeneousProject(t *testing.T) {
	...
	for _, file := range r25ObjectSlice(t, outline["files"]) {
	...
	showResult := r25CallSourceSymbols(t, ctx, session, map[string]any{
	...
	shown := r25SchemaMap(t, r25StructuredMap(t, showResult)["show"])
	...
	relations := r25StructuredMap(t, relationsResult)
	relationPayload := r25SchemaMap(t, relations["relations"])
	edges := r25ObjectSlice(t, relationPayload["relations"])
	if len(edges) != 1 {
	...
	contextOutput := r25StructuredMap(t, contextResult)
	contextPayload := r25SchemaMap(t, contextOutput["context"])
	items := r25ObjectSlice(t, contextPayload["items"])
	...
	index := r25SchemaMap(t, contextOutput["index"])
