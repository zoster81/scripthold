...
func TestR27Phase11RegistryActivatesCompositeProviders(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
	for _, language := range []string{"vue", "svelte", "astro", "php-html", "jsp", "jinja", "twig", "blade", "ejs"} {
		descriptor, ok := registry.Lookup(language)
		if !ok {
			t.Fatalf("Phase 11 provider %q missing from registry", language)
		}
		if descriptor.Analyzer == "" || !descriptor.Capabilities.SourceAnalysis || !descriptor.Capabilities.Composite {
			t.Fatalf("Phase 11 provider %q is not active composite support: %+v", language, descriptor)
		}
		if descriptor.AnalyzerVersion != "r27-p11-v1" || descriptor.AnalyzerStrategy != "native-masked-composite" {
			t.Fatalf("Phase 11 provider %q metadata = %+v", language, descriptor)
		}
		if analyzer, ok := AnalyzerFor(descriptor); !ok || analyzer == nil || analyzer.ID() != descriptor.Analyzer {
			t.Fatalf("Phase 11 provider %q cannot instantiate analyzer", language)
		}
	}
...
func TestR27Phase11DedicatedSuffixDetection(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase11PHPHTMLDetectionRequiresRealHostMarkup(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
