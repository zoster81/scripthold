...
func TestSourceIntelligenceHasNoHiddenExecutionOrNetworkImports(t *testing.T) {
...
func TestCancelledRefreshPublishesNoGeneration(t *testing.T) {
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		t.Fatal(err)
	}
	path := filepath.Join("project", "phase17", "cancel.ts")
	fact := indexFacts(t, registry, path, "typescript", "export class Cancelled {}\n")
	...
	options := ProjectIndexRefreshOptions{
		ScopeFingerprint:    indexDigest("phase17-cancel-scope"),
		AnalysisFingerprint: indexDigest("phase17-analysis"),
...
func BenchmarkAmbiguousDetection(b *testing.B) {
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		b.Fatal(err)
	}
