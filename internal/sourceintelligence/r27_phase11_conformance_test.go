...
func TestR27Phase11OpaqueHostAndTemplateRegionsDoNotLeakSymbols(t *testing.T) {
	tests := []struct {
		name      string
...
func TestR27Phase11ScriptStyleRegionsRemainInSourceOrder(t *testing.T) {
	text := `<style>.card { display: block; }</style>
<main id="hero"></main>
<script>function load() {}</script>`
	result, err := (VueAnalyzer{}).Analyze(context.Background(), phase9TestDocument("fixture.vue", text), phase3AnalyzeOptions(true, 128))
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase11UnsupportedEmbeddedLanguageIsExplicitPartialRegion(t *testing.T) {
