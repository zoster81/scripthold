...
func TestOpaqueRegionsDoNotEmitDeclarations(t *testing.T) {
	tests := []struct {
		name      string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			doc.Path = "fixture"
			result, err := tc.analyzer.Analyze(context.Background(), doc, testAnalyzeOptions(true, 128))
			if err != nil {
				t.Fatal(err)
			}
...
func TestMalformedStructuredDocumentsAreIncomplete(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			doc := sourceDocumentForScanner(tc.text)
			result, err := tc.analyzer.Analyze(context.Background(), doc, testAnalyzeOptions(true, 64))
			if err != nil {
				t.Fatal(err)
			}
