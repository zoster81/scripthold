...
func TestR27Phase10RegistryActivatesEveryProvider(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
	for _, language := range []string{
		"sql", "plsql", "graphql", "terraform", "nix", "proto",
		"vhdl", "verilog", "systemverilog", "assembly",
		"html", "xml", "css", "scss", "sass", "less",
		"json", "yaml", "toml", "markdown", "openapi", "ansible-yaml",
	} {
		descriptor, ok := registry.Lookup(language)
		if !ok {
			t.Fatalf("Phase 10 provider %q missing from registry", language)
		}
		if descriptor.Analyzer == "" || !descriptor.Capabilities.SourceAnalysis {
			t.Fatalf("Phase 10 provider %q is not active: %+v", language, descriptor)
		}
		if analyzer, ok := AnalyzerFor(descriptor); !ok || analyzer == nil {
			t.Fatalf("Phase 10 provider %q cannot instantiate analyzer", language)
		}
	}
...
func TestR27Phase10DistinctiveDetection(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase10GenericYAMLAndSQLRemainGenericWithoutDistinctiveEvidence(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
