...
func TestDynamicBEAMScriptingConformanceAcrossEncodingsAndDeterminism(t *testing.T) {
	tests := []struct {
...
func TestProductionOpaqueAndMultilineBoundaries(t *testing.T) {
	...
	t.Run("groovy-slashy-and-dollar-slashy", func(t *testing.T) {
		...
		result, err := (GroovyAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), testAnalyzeOptions(false, 64))
		if err != nil {
			t.Fatal(err)
		}
		names := sortedSymbolQualifiedNames(result.Analysis.Symbols)
	...
	t.Run("autohotkey-backtick-escaped-quote", func(t *testing.T) {
		...
		result, err := (AutoHotkeyAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), testAnalyzeOptions(false, 64))
		if err != nil {
			t.Fatal(err)
		}
		names := sortedSymbolQualifiedNames(result.Analysis.Symbols)
	...
	t.Run("elixir-multiline-function-scope", func(t *testing.T) {
		...
		result, err := (ElixirAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), testAnalyzeOptions(false, 64))
		if err != nil {
			t.Fatal(err)
		}
...
func TestMalformedCancellationAndLimits(t *testing.T) {
	malformed := []struct {
		name     string
	...
	for _, analyzer := range dynamicBEAMScriptingAnalyzers() {
	...
	for _, tc := range []struct {
		...
		{"perl", PerlAnalyzer{}, generatedPerl(1200)},
		{"lua", LuaAnalyzer{}, generatedLua(1200)},
		{"luau", LuauAnalyzer{}, generatedLua(1200)},
		{"elixir", ElixirAnalyzer{}, generatedElixir(1200)},
		{"erlang", ErlangAnalyzer{}, generatedErlang(1200)},
		{"gleam", GleamAnalyzer{}, generatedGleam(1200)},
		{"groovy", GroovyAnalyzer{}, generatedGroovy(1200)},
		{"shell", ShellAnalyzer{}, generatedShell(1200)},
		{"bash", BashAnalyzer{}, generatedShell(1200)},
		{"tcl", TclAnalyzer{}, generatedTcl(1200)},
		{"autohotkey", AutoHotkeyAnalyzer{}, generatedAHK(1200)},
	} {
...
func TestDetectionIsConservativeAndKeepsShellProvidersDistinct(t *testing.T) {
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		t.Fatal(err)
	}
...
func generatedPerl(count int) string {
...
func generatedLua(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedElixir(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedErlang(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedGleam(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedGroovy(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedShell(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
		fmt.Fprintf(&b, "f%04d() { :; }\n", i)
	}
...
func generatedTcl(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func generatedAHK(count int) string {
	var b strings.Builder
	for i := 0; i < count; i++ {
...
func dynamicBEAMScriptingAnalyzers() []SourceAnalyzer {
