...
func TestBasicDialectAnalyzersRemainDistinct(t *testing.T) {
	tests := []struct {
		language string
...
func TestRealWorldFreeBasicTypeAliasesDoNotOpenScopes(t *testing.T) {
...
func TestRealWorldVBDesignerKeywordAssignmentsDoNotOpenScopes(t *testing.T) {
	text := "VERSION 5.00\r\n" +
		...
		"Private Sub Run()\r\nEnd Sub\r\n"
	result, err := (VB6Analyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), testAnalyzeOptions(true, 256))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRealWorldVBPropertyProceduresUseDeclaredPropertyName(t *testing.T) {
	tests := []struct {
		name     string
		analyzer SourceAnalyzer
		text     string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := tc.analyzer.Analyze(context.Background(), sourceDocumentForScanner(tc.text), testAnalyzeOptions(true, 256))
			if err != nil {
				t.Fatal(err)
			}
			byName := symbolsByQualifiedName(result.Analysis.Symbols)
...
func TestDotNetAdjacentAnalyzers(t *testing.T) {
	tests := []struct {
		language string
		analyzer SourceAnalyzer
		text     string
...
func TestClassicASPDelegatesVBScriptAndJScript(t *testing.T) {
...
function JSHello(value) { return value; }
	...
	result, err := (ClassicASPAnalyzer{}).Analyze(context.Background(), document, testAnalyzeOptions(true, 128))
	if err != nil {
		t.Fatal(err)
	}
	if !result.Analysis.CoverageComplete {
...
func TestCompositeDotNetFormatsPreserveHostCoordinates(t *testing.T) {
	tests := []struct {
		language string
		analyzer SourceAnalyzer
		path     string
...
@code {
	...
	for _, tc := range tests {
		t.Run(tc.language, func(t *testing.T) {
			...
			result, err := tc.analyzer.Analyze(context.Background(), document, testAnalyzeOptions(true, 128))
			if err != nil {
				t.Fatal(err)
			}
			if !result.Analysis.CoverageComplete {
...
func TestBasicDialectsRemainCaseInsensitiveAndDoNotOverclaimSemanticRelations(t *testing.T) {
	...
	for _, language := range []string{"fsharp", "cpp-cli", "jscript-net", "cil", "powershell", "classic-asp", "aspnet-webforms", "razor", "blazor", "xaml"} {
		descriptor, _ := registry.Resolve(language)
		if descriptor.Capabilities.SemanticRelations {
			t.Fatalf("%s overclaims semantic relations: %+v", language, descriptor.Capabilities)
		}
...
func TestRepresentativeDependenciesRemainLiteralAndDeterministic(t *testing.T) {
