...
func TestR27Phase11CancellationAcrossNewProviders(t *testing.T) {
	for _, analyzer := range []SourceAnalyzer{
		...
		t.Run(analyzer.Language(), func(t *testing.T) {
			...
			cancel()
			_, err := analyzer.Analyze(ctx, phase9TestDocument("fixture", `<main id="hero"></main>`), phase3AnalyzeOptions(true, 32))
...
func TestR27Phase11CompositeRegionOutputIsBounded(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			options := phase3AnalyzeOptions(true, 128)
			result, err := tc.analyzer.Analyze(context.Background(), phase9TestDocument("fixture", tc.text), options)
			if err != nil {
				t.Fatal(err)
			}
...
func TestR27Phase11DotNetCompositeAggregateRegionBudgetAndMalformedClient(t *testing.T) {
	...
	options := phase3AnalyzeOptions(true, 128)
	result, err := (ASPNetWebFormsAnalyzer{}).Analyze(context.Background(), phase9TestDocument("fixture.aspx", mixed.String()), options)
	if err != nil {
		t.Fatal(err)
	}
	...
	malformed, err := (RazorAnalyzer{}).Analyze(context.Background(), phase9TestDocument("fixture.cshtml", `@functions { public void ServerRun() {} }<script>function broken() {}`), phase3AnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase11UnterminatedCompositeRegionsLowerCoverage(t *testing.T) {
	for _, tc := range []struct {
		name     string
		analyzer SourceAnalyzer
		text     string
