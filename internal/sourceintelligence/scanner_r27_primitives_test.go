...
func TestR27ScannerProfileIdentifierDelimiterAndDirectivePolicies(t *testing.T) {
...
func TestR27ScannerHeredocsHideDeclarationLookingTextAndSupportMultipleBodies(t *testing.T) {
	profile := ScannerProfile{
		Name:         "r27-shell",
...
func TestR27ScannerReportsAllPendingHeredocsWhenFirstIsUnterminated(t *testing.T) {
...
func TestR27LogicalLineBuilderHandlesBasicSeparatorsAndIndentation(t *testing.T) {
	vbText := "Class Demo\n    Dim total = 1 _\n        + 2 : Dim other = 3\nEnd Class\n"
	vbScan := scanSourceText(t, vbText, VBNetScannerProfile(), phase4ScannerLimits)
	...
	pyText := "class Demo:\n    def work():\n        return 1\nvalue = 2\n"
	pyScan := scanSourceText(t, pyText, PythonScannerProfile(), phase4ScannerLimits)
...
func TestR27KeywordScopePairingIsTopOnlyAndDeterministic(t *testing.T) {
...
func TestR27FixedAndFreeLineModelsPreserveOffsetsContinuationAndLabels(t *testing.T) {
...
func TestR27CompositeSegmentationAndMaskingPreserveUTF8Offsets(t *testing.T) {
...
func TestR27ScannerPrimitiveLimitsCancellationAndMalformedProfiles(t *testing.T) {
	...
	_, err = ScanSource(context.Background(), sourceDocumentForScanner("x"), invalid, phase4ScannerLimits)
	if operation.KindOf(err) != operation.KindInvalidInput {
		t.Fatalf("invalid delimiter profile = %v kind=%v", err, operation.KindOf(err))
	}
}

func FuzzR27ScannerPrimitiveProfiles(f *testing.F) {
