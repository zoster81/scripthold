...
func TestR27Phase9ClojureNamespaceCollectsAllStaticRequireVectors(t *testing.T) {
...
func TestR27Phase9StaticDependencyFormsRejectDynamicTargets(t *testing.T) {
	tests := []struct {
		name     string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := tc.analyzer.Analyze(context.Background(), phase9TestDocument("dynamic.fixture", tc.text), phase3AnalyzeOptions(false, 64))
			if err != nil {
				t.Fatal(err)
			}
