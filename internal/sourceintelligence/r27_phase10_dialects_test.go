...
func TestR27Phase10SQLDialectProfiles(t *testing.T) {
	tests := []struct {
		name string
		text string
		want []string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := (SQLAnalyzer{}).Analyze(context.Background(), phase9TestDocument("dialect.sql", tc.text), phase3AnalyzeOptions(true, 64))
			if err != nil {
				t.Fatal(err)
			}
...
func TestR27Phase10GenericHCLUsesTerraformProviderWithoutInventingTerraformSemantics(t *testing.T) {
	...
	result, err := (TerraformAnalyzer{}).Analyze(context.Background(), phase9TestDocument("service.hcl", "service \"api\" { port = 8080 }\nprovider \"aws\" {}\nlocals { answer = 42 }\n"), phase3AnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase10AssemblyDialectLabels(t *testing.T) {
	tests := []struct {
		name string
		text string
		want []string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := (AssemblyAnalyzer{}).Analyze(context.Background(), phase9TestDocument("fixture.asm", tc.text), phase3AnalyzeOptions(true, 64))
			if err != nil {
				t.Fatal(err)
			}
			names := sortedSymbolQualifiedNames(result.Analysis.Symbols)
...
func TestR27Phase10DistinctiveMarkersIgnoreOpaqueText(t *testing.T) {
	registry, err := NewLanguageRegistry(defaultLanguageDescriptors())
	if err != nil {
		t.Fatal(err)
	}
