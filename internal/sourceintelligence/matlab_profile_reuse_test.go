...
func TestMATLABAnalyzerProfileReusePreservesFactoryIsolationAndConcurrency(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "matlab", analyzer: MATLABAnalyzer{}, path: "fixture.m", text: "% comment\nfunction y = run(x)\n  y = x + 1;\nend\n"},
		{name: "octave", analyzer: OctaveAnalyzer{}, path: "fixture.m", text: "# comment\nfunction y = run(x)\n  y = x + 1;\nendfunction\n"},
	}
	...
	for _, name := range []string{"matlab", "octave"} {
		publicProfile := MATLABScannerProfile(name)
		if len(publicProfile.Keywords) == 0 || len(publicProfile.LineComments) == 0 || len(publicProfile.BlockComments) == 0 || len(publicProfile.Strings) == 0 {
			t.Fatalf("%s scanner profile unexpectedly lacks mutable rule slices", name)
		}
