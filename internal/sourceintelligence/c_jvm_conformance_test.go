...
func TestConformanceAcrossEncodingsAndUnicode(t *testing.T) {
	tests := []struct {
...
func TestOpaqueAdvancedStringFamiliesAndPreprocessorTruthfulness(t *testing.T) {
	cppText := "class Real {};\nconst char* raw = R\"DELIM(\" class RawFake { void Nope(); } \"\nsecond line)DELIM\";\n"
	cpp, err := (CPPAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(cppText), testAnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
	...
	java, err := (JavaAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(javaText), testAnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
	...
	kotlin, err := (KotlinAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(kotlinText), testAnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
	...
	conditional, err := (CAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(conditionalText), testAnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCJVMGeneratedSourcesRespectSymbolLimits(t *testing.T) {
	cases := []struct{ language, text string }{
		{language: "c", text: generatedCSource(1200)},
		{language: "cpp", text: generatedCPPSource(1200)},
		{language: "java", text: generatedJavaSource(1200)},
		{language: "kotlin", text: generatedKotlinSource(1200)},
	}
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		t.Fatal(err)
	...
	for _, testCase := range cases {
		t.Run(testCase.language, func(t *testing.T) {
			...
			result, err := analyzer.Analyze(context.Background(), document, testAnalyzeOptions(false, 128))
			if err != nil {
				t.Fatal(err)
			}
...
func generatedCSource(count int) string {
...
func generatedCPPSource(count int) string {
	var builder strings.Builder
...
func generatedJavaSource(count int) string {
	var builder strings.Builder
...
func generatedKotlinSource(count int) string {
	var builder strings.Builder
	builder.WriteString("package generated\n")
	for index := 0; index < count; index++ {
