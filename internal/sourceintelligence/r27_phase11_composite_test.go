...
func TestR27Phase11ProviderIdentityAndHostCoordinatePreservation(t *testing.T) {
	tests := []struct {
		language string
...
<style>.card { display: block; }</style>`,
	...
	for _, tc := range tests {
		t.Run(tc.language, func(t *testing.T) {
			document := phase9TestDocument("fixture", tc.text)
			...
			for qualified, kind := range tc.want {
				...
				if wantLanguage := phase11ExpectedSymbolLanguage(tc.language, qualified); symbol.Language != wantLanguage {
...
func phase11ExpectedSymbolLanguage(provider, qualified string) string {
	switch provider {
...
func TestR27Phase11ExistingDotNetCompositesKeepHostCoordinates(t *testing.T) {
	tests := []struct {
		language string
		analyzer SourceAnalyzer
		text     string
	...
	for _, tc := range tests {
		t.Run(tc.language, func(t *testing.T) {
			document := phase9TestDocument("fixture", tc.text)
			result, err := tc.analyzer.Analyze(context.Background(), document, phase3AnalyzeOptions(true, 64))
			if err != nil {
				t.Fatal(err)
			}
			found := false
