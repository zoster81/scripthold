...
func TestDetectionRoutesScalaAndFlowWithoutContaminatingJavaScript(t *testing.T) {
...
func TestScalaAndFlowExposeDistinctStructuralProviders(t *testing.T) {
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		t.Fatal(err)
	}
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.language, func(t *testing.T) {
			...
			result, err := analyzer.Analyze(context.Background(), sourceDocumentForScanner(tc.text), testAnalyzeOptions(true, 256))
			if err != nil {
				t.Fatal(err)
			}
...
func TestScalaAndFlowOpaqueMalformedLimitsCancellationAndEncodings(t *testing.T) {
	registry, err := DefaultLanguageRegistry()
	if err != nil {
		t.Fatal(err)
	}
	for _, tc := range []struct {
		...
		t.Run(tc.language, func(t *testing.T) {
			...
			result, err := analyzer.Analyze(context.Background(), document, testAnalyzeOptions(true, 128))
			if err != nil {
				t.Fatal(err)
			}
			...
			cancel()
			if _, err := analyzer.Analyze(ctx, sourceDocumentForScanner("class A {}\n"), testAnalyzeOptions(false, 16)); operation.KindOf(err) != operation.KindCancelled {
			...
			limited, err := analyzer.Analyze(context.Background(), sourceDocumentForScanner(generated.String()), testAnalyzeOptions(false, 128))
			if err != nil {
				t.Fatal(err)
			}
