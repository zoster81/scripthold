...
func TestDynamicBEAMAndScriptingAnalyzersExposeDistinctNativeStructure(t *testing.T) {
	tests := []struct {
		language string
...
func TestOpaqueDataAndQuotingDoNotLeakDeclarations(t *testing.T) {
	tests := []struct {
		name     string
		analyzer SourceAnalyzer
		text     string
...
func TestRealWorldShellMultilineQuotedStringsRemainOpaque(t *testing.T) {
...
func TestDynamicBEAMScriptingCapabilityCeilings(t *testing.T) {
	...
	for _, language := range []string{"perl", "lua", "luau", "elixir", "erlang", "gleam", "groovy", "shell", "bash", "tcl", "autohotkey"} {
