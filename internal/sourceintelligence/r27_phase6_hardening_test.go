...
func TestR27Phase6JScriptNetEvidenceUsesHostCoordinates(t *testing.T) {
	text := `package Demo {
import System;
...
`
	...
	result, err := (JScriptNetAnalyzer{}).Analyze(context.Background(), document, phase3AnalyzeOptions(true, 128))
	if err != nil {
		t.Fatal(err)
	}
...
func TestR27Phase6XAMLRequiresBalancedAttributeQuotes(t *testing.T) {
	text := `<Window x:Class="Demo.Bad' xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"><Grid x:Name='Root" /></Window>`
	result, err := (XAMLAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), phase3AnalyzeOptions(true, 64))
	if err != nil {
		t.Fatal(err)
	}
