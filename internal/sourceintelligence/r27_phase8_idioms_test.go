...
func TestR27Phase8CommonIdiomaticDeclarations(t *testing.T) {
	t.Run("lua-assigned-function", func(t *testing.T) {
		text := "local assigned = function(value) return value end\n"
		result, err := (LuaAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), phase3AnalyzeOptions(true, 64))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("luau-assigned-function", func(t *testing.T) {
		...
		result, err := (LuauAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), phase3AnalyzeOptions(true, 64))
		if err != nil {
			t.Fatal(err)
		}
		if symbol, ok := symbolsByQualifiedName(result.Analysis.Symbols)["assigned"]; !ok || symbol.Kind != SymbolKindFunction {
	...
	t.Run("gleam-pub-opaque-type", func(t *testing.T) {
		...
		result, err := (GleamAnalyzer{}).Analyze(context.Background(), sourceDocumentForScanner(text), phase3AnalyzeOptions(true, 64))
		if err != nil {
			t.Fatal(err)
		}
...
func TestR27Phase8CustomOpaqueConstructsReportIncompleteWhenUnterminated(t *testing.T) {
	tests := []struct {
		name     string
