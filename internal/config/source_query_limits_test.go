...
type sourceQueryLimitExpectation struct {
	field        string
	environment  string
	defaultValue int64
	hardMaximum  int64
...
var sourceQueryLimitContract = []sourceQueryLimitExpectation{
...
func TestSourceQueryLimitsDefaults(t *testing.T) {
	cfg := LoadFromEnvironment(func(string) string { return "" })
	for _, expectation := range sourceQueryLimitContract {
		if got := sourceQueryLimitValue(t, cfg, expectation.field); got != expectation.defaultValue {
			t.Errorf("%s default = %d, want %d", expectation.field, got, expectation.defaultValue)
		}
	}
}

func TestSourceQueryLimitsEnvironmentOverrides(t *testing.T) {
	for _, expectation := range sourceQueryLimitContract {
		t.Run(expectation.field, func(t *testing.T) {
			...
			if got := sourceQueryLimitValue(t, cfg, expectation.field); got != override {
...
func TestSourceQueryLimitsRejectValuesAboveHardMaximum(t *testing.T) {
	for _, expectation := range sourceQueryLimitContract {
		t.Run(expectation.field, func(t *testing.T) {
			cfg := LoadFromEnvironment(func(name string) string {
				if name == expectation.environment {
...
func sourceQueryLimitValue(t *testing.T, cfg *Config, field string) int64 {
	t.Helper()
