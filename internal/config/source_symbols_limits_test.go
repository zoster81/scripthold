...
type sourceSymbolsLimitExpectation struct {
	field        string
	environment  string
	defaultValue int64
	hardMaximum  int64
...
var sourceSymbolsLimitContract = []sourceSymbolsLimitExpectation{
...
func TestSourceSymbolsLimitsDefaults(t *testing.T) {
	cfg := LoadFromEnvironment(func(string) string { return "" })
	for _, expectation := range sourceSymbolsLimitContract {
		if got := sourceSymbolsLimitValue(t, cfg, expectation.field); got != expectation.defaultValue {
			t.Errorf("%s default = %d, want %d", expectation.field, got, expectation.defaultValue)
		}
	}
}

func TestSourceSymbolsLimitsEnvironmentOverrides(t *testing.T) {
	for _, expectation := range sourceSymbolsLimitContract {
		t.Run(expectation.field, func(t *testing.T) {
			...
			if got := sourceSymbolsLimitValue(t, cfg, expectation.field); got != override {
...
func TestSourceSymbolsLimitsRejectValuesAboveHardMaximum(t *testing.T) {
	for _, expectation := range sourceSymbolsLimitContract {
		t.Run(expectation.field, func(t *testing.T) {
			cfg := LoadFromEnvironment(func(name string) string {
				if name == expectation.environment {
...
func sourceSymbolsLimitValue(t *testing.T, cfg *Config, field string) int64 {
	t.Helper()
	if cfg == nil {
