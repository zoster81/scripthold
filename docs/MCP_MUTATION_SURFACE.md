...
- **R24 — Safe filesystem operations (ACTIVE):** governed by [SAFE_FILESYSTEM_OPERATIONS.md](SAFE_FILESYSTEM_OPERATIONS.md), with typed preview/apply packages for coordinated create/copy/move/delete/directory operations without arbitrary shell commands; the source changes are implemented, available local gates and activated-candidate connector preview/apply acceptance pass, while only required native Linux/macOS namespace verification remains pending;
