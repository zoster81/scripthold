...
`COMPLETE` release-scoped contract for R22. All implementation and verification phases are complete, and Scripthold `2.2.0` was published on 2026-08-11 from the exact CI-gated release commit. The current public and MCP Registry release is `2.2.0`; deployment remains a separately governed operation.
...
11. Complete full repository, six-target, native/container, deterministic GoReleaser packaging, and local release-candidate verification. **Completed locally on 2026-08-11:** six target binaries and 114 cross-platform test binaries compile; Windows amd64 passes native runtime smoke; the Linux amd64 container passes the documented hardened stdio and direct-TLS HTTP gate; two physically isolated GoReleaser snapshots produce byte-identical six raw binaries, six platform archives, and `checksums.txt` with deterministic archive metadata and exact SHA-256 coverage; the complete Go/race/static/vulnerability, deterministic fuzz, workflow, release-metadata, documentation, and Gitleaks gates are green. MCPB bundle creation, MCPB checksums, the final MCPB-backed Registry manifest, and Registry publication remain excluded from local Phase 11: they are produced only by GitHub release workflows after tagging and must never be generated or simulated locally.**
12. Only after all prior gates are green, prepare and publish Scripthold `2.2.0` through the normal exact-commit release procedure. **Completed on 2026-08-11:** exact commit `f6ade5e3e47eff03a2d728c73a15409786765adb` passed the full push-event release-candidate CI gate, annotated tag `v2.2.0` was created on that commit, and the GitHub Release, GitHub-only MCPB assets, and MCP Registry publication all completed successfully.
...
- GoReleaser output passes the deterministic local release-candidate gates, while the GitHub MCPB/Registry workflow definitions and non-artifact-producing tests are green; actual MCPB bundles, MCPB checksums, the final MCPB-backed Registry manifest, and Registry publication are GitHub-only post-tag outputs and are never produced locally;
- the release candidate is an exact clean commit with a dated `2.2.0` changelog entry before tagging.

GitHub Release and Registry publication are complete for `2.2.0`. Deployment, active rollback, and restoration remain separate final operations governed by `PUBLISHING.md`; they have not been performed for `2.2.0` and must not be inferred from publication success alone.

## Completion record

R22 completed and shipped on 2026-08-11. The published release exposes 168 canonical encodings with the conservative detection and explicit-only boundaries defined above, includes full UTF-32 LE/BE text-pipeline support, and preserves the pure-Go runtime boundary. The final exact commit passed the repository's complete cross-platform CI gate before tagging; local deterministic GoReleaser verification and the GitHub release pipeline agreed on the six raw binaries, six platform archives, and checksum contract. MCPB packaging, MCPB checksums, final Registry-manifest generation/validation, and Registry publication occurred only in the GitHub workflows after tagging.
