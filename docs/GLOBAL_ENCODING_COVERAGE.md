...
`ACTIVE` release-scoped contract for R22. Phases 1–10 are implemented and Phase 11 local release-candidate verification is technically complete. The current public release remains `2.1.1`; R22 stays active until exact clean `2.2.0` release preparation and the separately governed GitHub publication gates are completed.
