...
- one authoritative tool catalog over stdio and Streamable HTTP: 36 tools in the active R27 Unreleased source tree and 30 in the public `2.2.0` release, plus 3 guided prompts;
...
Scripthold `2.2.0` is the current public release. R22 completed the 168-codec global encoding expansion and full UTF-32 LE/BE text support. **R23 completed on 2026-08-12**, delivering truthful separation of read-only preparation from mutation. **R24 completed on 2026-08-13**, delivering the 34-tool Unreleased safe-filesystem package surface with bounded `filesystem_package` preview/apply, exact recursive copy/delete, same-volume native move, backup-before-loss deletion, and deterministic partial-state evidence. **R25 completed on 2026-08-13**, adding `source_symbols` as the 35th Unreleased tool with native Go/C#/VB.NET/Python declaration navigation, Classic ASP segmentation/delegation, evidence-qualified detection, decoded-source coordinates, bounded request-scoped analysis, and no external parser/compiler/LSP runtime dependency. **R26 completed on 2026-08-14**, adding explicit offline evidence-preserving backup-store recovery into a separate fully audited destination without changing normal fail-closed startup or R19 diagnostics. The completed R25 and R26 contracts are in [SOURCE_INTELLIGENCE.md](SOURCE_INTELLIGENCE.md) and [BACKUP_RECOVERY.md](BACKUP_RECOVERY.md). R27 was activated on 2026-08-14; Phase 1 froze `source_query` as the 36th Unreleased tool, Phase 2 completed the reusable profile-driven native scanner/recognizer foundation plus the mechanically verified [language capability matrix](LANGUAGE_CAPABILITIES.md), Phase 3 added C/C++/Java/Kotlin, Phase 4 JavaScript/JSX/TypeScript/TSX/Rust, Phase 5 PHP/Ruby/Swift/Pascal/Delphi, and Phase 6 the distinct Basic/.NET/composite providers for VB6, VBA, VBScript, QBasic, classic BASIC, FreeBASIC, PureBasic, F#, C++/CLI, JScript.NET, CIL, PowerShell, ASP.NET Web Forms, Razor, Blazor, and XAML while extending Classic ASP to VBScript/JScript delegation. Phase 7 added distinct MQL4/MQL5, Objective-C/Objective-C++, Dart, D, Zig, Nim, Solidity, Apex, AL, and Arduino providers. Phase 8 added distinct Perl, Lua, Luau, Elixir, Erlang, Gleam, Groovy, POSIX shell, Bash, Tcl, and AutoHotkey providers. Phase 9 added distinct Fortran, COBOL, Ada, MATLAB, Octave, Julia, R, Haskell, OCaml, Common Lisp, Clojure, and Emacs Lisp providers. Phase 10 added the distinct SQL, PL/SQL, GraphQL, Terraform/HCL, Nix, Protocol Buffers, VHDL, Verilog, SystemVerilog, Assembly, HTML, XML, CSS, SCSS, Sass, Less, JSON, YAML, TOML, Markdown, OpenAPI, and Ansible-oriented YAML providers, with format-appropriate structural entities rather than fictitious programming-language semantics. Phase 11 added distinct Vue, Svelte, Astro, PHP/HTML, JSP, Jinja, Twig, Blade, and EJS providers and completed host HTML/client JavaScript/CSS delegation for ASP.NET Web Forms, Razor, and Blazor. Phase 12 completed the bounded memory-only project resolver over normalized analyzer facts, including deterministic symbol/dependency indexes, staged cross-file resolution and explicit ambiguity across the required ecosystem subset. Phase 13 integrated that resolver into public `source_query` structural search and supported project relations with bounded BFS trace/impact and Tarjan dependency-cycle detection while retaining truthful evidence/resolution and graph limits. Phase 14 added deterministic fingerprint-verified `source_query context` assembly with target/enclosing/dependency/type priorities, exact decoded UTF-8 budgeting, body-to-signature degradation, signature-only deeper relations, all-or-error explicit target retention, and fail-closed current-source revalidation before text is returned. Phase 15 added bounded process-local incremental `source_query` generations with content/analyzer/configuration fingerprint invalidation, unchanged-fact reuse, conservative whole-model relationship recomputation after changes, exact retained stale-generation selection, bounded scope/history retention, and no retained complete source bodies or persistent storage. The registry still exposes 99 active production providers; every active source-analysis provider now truthfully advertises process-local `index`, while `project-ref`/`project-def` remain promoted only for the verified C++, Java, Kotlin, TypeScript, Rust, Ruby and Delphi subset, with Java `impl` proven separately. Unsupported semantic relations remain unclaimed and persistent on-disk indexing remains future work. `.mqh` remains ambiguity-safe between MQL4/MQL5 and generic `.m` remains ambiguity-safe among Objective-C, MATLAB, and Octave unless independent evidence resolves it. Later rows remain unimplemented rather than being counted as support. Phases 0-15 are complete; Phase 16 capability/corpus completion is next. The active R27 design and contract are in [MULTILANGUAGE_CODE_INTELLIGENCE.md](MULTILANGUAGE_CODE_INTELLIGENCE.md).
