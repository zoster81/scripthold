...
R25 completed on 2026-08-13 after focused conformance and adversarial gates, cross-encoding corpora, deterministic concurrency/benchmarks, direct/HTTP/stdio MCP acceptance, complete normal and CGO race suites, Go vet, Staticcheck, govulncheck, deterministic scanner fuzzing, catalog/documentation checks, and compile-only builds for Windows/Linux/macOS on amd64/arm64. Before delivery, an additional real-world gate expanded acceptance to at least eight independent public origins per canary: 41 public upstreams, 8,945 selected source files and 125,607 retained symbols were analyzed twice with deterministic range/hierarchy invariants, while intentionally malformed or unsupported source remained explicitly partial. The final R27 compatibility review found no breaking public-schema redesign requirement. Repository delivery then exposed only cross-platform assumptions in test/smoke path handling; those test-only fixes preserved production behavior. The final pushed R25 head `1c558a8cf37c634f6ef4b8a7e3af2d6879c33526` passed native Windows/Ubuntu/macOS tests and smoke, container smoke, static analysis, deterministic fuzzing, all six supported cross-builds, and the aggregate `Release candidate` gate. No tag or public release was created. The completed contract and verification record remain in [SOURCE_INTELLIGENCE.md](SOURCE_INTELLIGENCE.md).

## R26 — Evidence-preserving backup recovery

Added explicit offline `backup-store recover-plan` / `recover-apply` recovery for an existing backup store without weakening normal fail-closed startup or R19 read-only diagnostics. Recovery treats the source as immutable evidence, emits a strict deterministic persisted plan, revalidates that plan under the exclusive existing-store lock, copies only fully verified manifest/object records into a separate staged destination with a fresh store identity, rebuilds derived state, requires a complete full audit before same-parent no-replace promotion, and writes a strict path-free provenance report. Missing/corrupt records, orphan objects, unknown residue, stale plans, collisions, and interrupted work remain explicit evidence rather than being silently repaired or invented.

R26 completed on 2026-08-14. Local verification covered strict codecs/fuzzing, mutation-negative source snapshots, cancellation/failure injection, stale/replacement races, Windows path/reparse/8.3 behavior, native external-process restart/apply/full-diagnose smoke, complete normal/race/static/vulnerability gates, and six-target compilation. The implementation commit `898c568b184b0982053a2a7c6ddb99f9e92fe9dd` then passed exact push-event CI run `31806897971`, including native Windows/Ubuntu/macOS regression and smoke jobs, module verification, Staticcheck/govulncheck/workflow lint, deterministic fuzz smoke, all six supported cross-builds, container smoke, and the aggregate `Release candidate` gate. No tag or public release was created. The completed contract and verification record remain in [BACKUP_RECOVERY.md](BACKUP_RECOVERY.md).

## R27 — Broad multi-language code intelligence

Extended the native R25 source-intelligence architecture into the read-only `source_query` search/relations/context surface and a completed 101-provider approved catalog across 103 registry rows. R27 adds profile-driven native scanners and recognizers across mainstream, legacy, scientific, functional, scripting, hardware, data/infra, document and composite/template families; bounded cross-file project resolution and evidence-qualified structural relations; deterministic graph search/cycles; fingerprint-verified task-context assembly; and bounded process-local incremental index generations with stale-generation binding and current-source revalidation before text materialization. The final provider wave added distinct Scala and Flow analyzers while leaving Dockerfile and Make as explicit auxiliary inactive metadata rather than counting detector-only rows as production support.

R27 completed on 2026-08-16. Verification included provider-specific cross-encoding/malformed/opaque/limit/cancellation corpora, mechanically generated capability-matrix checks, project/context/index TDD, repository-scale concurrency and security tests, repeatable cold/warm/incremental/graph/context/mixed-language benchmarks, a heterogeneous in-process MCP workflow, a full normal suite, a complete repository race suite, Go vet, Staticcheck, govulncheck with no vulnerabilities found, prescribed Node metadata tests, and a 5,000-execution Scala/Flow fuzz campaign. The connector workflow exposed and TDD-fixed a Scala 3 detection ambiguity; a separate Flow RED test tightened `opaque type` normalization without erasing a legitimate identifier named `opaque`. Product builds, cross-builds, publication, deployment and runtime migration were not part of this source-completion operation. The completed contract and verification record remain in [MULTILANGUAGE_CODE_INTELLIGENCE.md](MULTILANGUAGE_CODE_INTELLIGENCE.md).

Scripthold `3.0.0` was published on 2026-08-17 from exact commit `ea6ff42f20de1a162e7e45860347d8b0001a8a1c` after the push-event `Test Suite` aggregate `Release candidate` and CodeQL gates succeeded. GoReleaser published six raw binaries, six platform archives, and `checksums.txt`; GitHub-only publication then added six MCPB bundles plus `mcpb-checksums.txt` and published MCP Registry version `3.0.0` for `io.github.zoster81/scripthold`. Post-publication verification matched all 12 normal GoReleaser assets to `checksums.txt` and executed the published Windows amd64 binary as version `3.0.0`. This release delivers the completed R23-R27 scope; operator deployment remains a separate action.

## R28 — Engine hygiene

Reduced demonstrated implementation debt without redesigning the public MCP surface. The deprecated package-level backup-store bridge now reuses the R23 read implementation where output equivalence was proven while retaining legacy validation, disabled-store behavior, and separate apply authority. Four unexported production helpers used only by tests were removed, while non-equivalent edit/package/encoding/BOM compatibility paths and broad legacy-encoding support were deliberately retained. Thirty-one historical phase/R27-named source-intelligence production files were renamed by current responsibility with byte-identical content.

R28 completed on 2026-08-18. Verification included focused compatibility/encoding/BOM/helper tests, byte-for-byte rename checks, complete normal repository tests, affected-package CGO race tests, Go vet, golangci-lint, Staticcheck, govulncheck with no vulnerabilities found, source-server smoke, project-identity/catalog checks, Gitleaks history and working-tree scans, documentation/link/diff hygiene, and representative benchmarks plus CPU profiling. The measurements established a baseline but did not demonstrate a single R28 performance bottleneck that justified speculative algorithm or caching changes, so no performance code change was made. Product builds, cross-builds, release actions, deployment, launcher changes, and runtime restart were outside this source-completion operation. The completed contract and verification record remain in [ENGINE_HYGIENE.md](ENGINE_HYGIENE.md).

Scripthold `3.1.0` was published on 2026-08-18 from exact commit `6b4c9a904caa6b357b3adfb95d1dd8b8e7782c02`. The push-event `Test Suite` run `32134496821` and its aggregate `Release candidate` job completed successfully, CodeQL run `32134496911` succeeded, and Release workflow `32135041516` completed release-candidate attestation, normal GoReleaser publication, GitHub-only MCPB publication, and MCP Registry publication. The public release exposes six raw binaries, six platform archives, six GitHub-produced MCPB bundles, `checksums.txt`, and `mcpb-checksums.txt`; MCP Registry version `3.1.0` is published for `io.github.zoster81/scripthold`. Publication remains separate from operator deployment.

Scripthold `3.1.5` was published on 2026-08-19 from exact commit `646976eceaa8905fcb136760a5f8ae94500a17c4` after completion of the cross-cutting test/build/CI architecture optimization that preceded R29. The maintenance release preserves the 36-tool public MCP surface while hardening verification ownership, deterministic harness behavior, shared fuzz orchestration, fail-closed CI evidence tiers, and local/GitHub release qualification. Hosted qualification also exposed and corrected a Unicode byte-offset panic in Classic ASP/ASP.NET/Razor composite scanning and a non-SLA Windows durable-task smoke watchdog. Push-event Test Suite `32198326726` completed with aggregate `Release candidate=success`, CodeQL `32198326729` succeeded, and Release workflow `32198677880` completed release-candidate verification, normal GoReleaser publication, GitHub-only MCPB publication, and MCP Registry publication successfully. Publication remains separate from operator deployment.
