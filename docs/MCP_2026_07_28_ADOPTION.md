...
Phase 3 implements the reviewed receiving middleware gate. By default it rejects `server/discover` before SDK dispatch only when client roots are enabled and startup directories are absent; configured-root and roots-disabled sessions use normal modern discovery. R21 adds the stdio-only `MCP_STDIO_LEGACY_HANDSHAKE=1` compatibility override for generic intermediaries that probe discovery but then send legacy `initialize` on the same persistent connection. The override rejects discovery before the SDK can populate initialization state, allowing deterministic legacy fallback without making duplicate initialization valid. It does not make one persistent stdio child a substitute for independent remote connector lifecycles; the public OpenAI Secure MCP Tunnel quick start therefore uses authenticated loopback Streamable HTTP. Streamable HTTP ignores this setting. The implementation does not fork the SDK, duplicate the server, or broaden filesystem authority.
