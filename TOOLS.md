...
Scripthold `3.2.0` exposes an authoritative 38-tool catalog and 3 guided prompts. The catalog is transport-independent. Stdio and Streamable HTTP expose the same schemas, annotations, process-wide allowed directories, limits, execution policy, typed errors, and prompt workflows; modern HTTP requests are stateless while retained legacy HTTP sessions remain stateful. Transport setup and security differ, but tool behavior does not; see [README.md](README.md), [docs/PROJECT_DIRECTION.md](docs/PROJECT_DIRECTION.md), [docs/HTTP_SECURITY.md](docs/HTTP_SECURITY.md), and [docs/DURABLE_TASKS.md](docs/DURABLE_TASKS.md).
...
Prepare one exact edit of an existing text file **without persistent mutation**. `edit_file` accepts only `action: "preview"`; mutation is performed separately by `edit_file_apply` using the returned `previewId`. Supply either `edits` or one strict single-file unified `patch`, never both.
...
Review or prepare a strict versioned package of coordinated edits to existing regular files **without persistent mutation**. `patch_package` accepts only `inspect`, `dryRun`, and `verify`; package application is performed separately by `patch_package_apply`.
...
The production surface contains 101 active approved target-catalog providers across 103 total registry rows; only auxiliary Dockerfile and Make metadata remain intentionally inactive and are not counted as production support. The authoritative per-provider capability projection is [docs/LANGUAGE_CAPABILITIES.md](docs/LANGUAGE_CAPABILITIES.md); it is generated from the native registry and mechanically checked for drift. Go uses the standard-library AST; the remaining programming-language providers use bounded native scanners, structural recognizers, or offset-preserving adapters. Composite providers preserve host coordinates and delegate only explicitly supported embedded regions. Dialect and shared-extension ambiguity remains fail-closed unless independent evidence resolves it. Bounded process-local incremental indexing is available for active source-analysis providers, while project/semantic relationship claims remain capability-specific and are never inferred merely from provider activation. Unsupported, ambiguous, partial, or truncated coverage is reported explicitly rather than guessed. No external parser/compiler/LSP process, network lookup, persistent code index, or complete-source cache is used.
...
`source_query` is the compact read-only contract for structural source search, supported project relations, fingerprint-verified task-context assembly, and bounded process-local incremental index generations over normalized analyzer facts. `textual` and `lexical` search remain `UNSUPPORTED` because `grep_text_files` is the decoded text-search surface. Index generation/fingerprint binding adds no persistent source-index storage.
...
**`search`:** requires non-empty `query` and `mode: "textual" | "lexical" | "structural"`; optional `match` is `exact`, `prefix`, or `contains`. `structural` operates over normalized symbols, dependencies, and resolved structural relationships without rescanning raw source. `textual` and `lexical` remain explicitly `UNSUPPORTED`. `kinds`, `evidence`, and `maxResults` are search-specific. Evidence strength uses the ordered vocabulary `textual`, `lexical`, `structural`, `scope-resolved`, `project-resolved`, and `semantic`; a result may never claim a stronger level than the native analyzer proves.

**`relations`:** requires `relation`, whose vocabulary is `dependencies`, `dependents`, `references`, `definitions`, `inheritance`, `implementations`, `overrides`, `callers`, `callees`, `trace`, `impact`, or `cycles`. Implemented relations include dependency/dependent adjacency, resolved structural references/definitions, inheritance, proven implementations, bounded dependency trace/impact, and dependency cycles. `callers`, `callees`, and `overrides` return `UNSUPPORTED` until analyzers provide trustworthy corresponding facts rather than name guesses. `trace` requires both `subject` and `target`; `cycles` accepts neither selector and does not accept `maxDepth`; the other relation kinds require `subject` and reject `target`. Relation queries may use `evidence`, `maxResults`, `maxNodes`, `maxEdges`, and—where meaningful—`maxDepth`.
...
**`context`:** requires one to 32 fingerprint-bound `targets` and positive `budgetBytes`; optional `bodyPolicy` is `prefer` or `signatures-only`, with `maxItems` and `maxDepth` as additional bounds. The deterministic planner runs against a coherent project generation; embeddings and persistent storage remain absent. Explicit symbol/position targets are all-or-error under byte/item pressure; `prefer` retains a complete declaration body when it fits and otherwise degrades to the bounded declaration signature. Priority is `target`, `enclosing`, `direct-dependency`, `direct-related-body`, `direct-related-signature`, `reverse-or-type-relation`, then `deeper-relation`; deeper relations are always signatures. Position selectors that hit resolved project references target their resolved definition candidates and preserve ambiguity rather than guessing. Before any text is returned—even when `stalePolicy=allow` selected retained metadata—the handler reopens the authorized current source, compares the authoritative fingerprint with the planned fingerprint, and slices only that verified decoded snapshot; source drift returns `CONFLICT`. `usedBytes` counts the actual UTF-8 bytes in returned decoded `ContextItem.text`, and coverage/truncation reports budget, item, depth, or representation degradation honestly. Direct caller/callee body/signature candidates remain absent until analyzers expose trustworthy call facts rather than name matches.

These configurable ceilings are fail-closed: invalid, non-positive, overflowing, or above-hard-maximum environment values fall back only as defined by the current configuration policy. Client-provided limits can only lower the effective configured ceiling. Existing source input/file/byte/request/output limits continue to apply as well.
...
Read and review the optional persistent backup store, or prepare restore/GC capabilities, **without persistent mutation**. `backup_store` is read-only; restore and GC application are separate tools. The tool remains registered when the store is disabled so operators can inspect policy state consistently.
...
Prepare exact encoding-conversion bytes for one file or a bounded batch **without persistent mutation**. `convert_encoding` requires `dryRun: true`; application is performed separately by `convert_encoding_apply` using the returned capability. Every target must be fully representable and prepared successfully before a capability is returned.
...
Detect BOM state or prepare an exact BOM mutation **without persistent mutation**. Mutating add/strip operations return a preview capability and are applied separately by `manage_bom_apply`.
...
The global `MCP_CALL_MAX_SYNC_SECONDS` cooperative deadline still applies to every public tool, including tools that are not eligible for independent execution. When the durable deferred-operation store is configured, requests using protocol `2026-07-28` or later may opt into `io.modelcontextprotocol/tasks`; an eligible slow `tools/call` then exposes the same durable operation as a native task, observed through `tasks/get`, acknowledged through `tasks/update` when client input responses are supplied, and cooperatively cancelled through `tasks/cancel`. The current deferred engine never enters `input_required`, so `tasks/update` validates the known task and required `inputResponses` field, ignores non-outstanding response keys, and does not mutate execution state. `deferred_operation` remains the compatibility surface for legacy sessions and for oversized completed responses, which are not native tasks.
